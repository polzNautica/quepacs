    ...
    <div className="items-center gap-2 grid lg:grid-cols-4 grid-cols-2">
      {pakej.map((item) => (
        <Card className={`w-full dark:bg-default-100/50 p-2 shadow-sm h-full border-1 border-default-200 overflow-hidden ${
          ...
          <Icon icon="line-md:star-twotone" className={`absolute top-1 right-1+ shadow-sm shadow-warning bg-warning rounded-full text-white animate-pulse ${item.title === "Zamrud 1" ? "block" : "hidden"}`}/>
        </Card>
      ))}  
      </div>
